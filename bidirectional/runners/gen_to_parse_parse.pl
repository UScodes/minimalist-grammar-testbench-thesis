...
main :-
    ...
    forall(
        ...
        (
            prepare_parser_tokens(GenTokens, ParserTokens),
            run_parse_case(ParserTokens, ParseStatus, ParsedSem, ParseTree),

            format(
                ParseS,
                "gen_to_parse_parsing_case(~q, ~q, ~q, ~q, ~q, ~q, ~q).~n",
                [CaseId, Sem, GenStatus, GenTokens, ParserTokens, ParseStatus, ParsedSem]
            ),
...
log_run_configuration :-
    ...
    format(
        "~n[gen_to_parse_parsing_stage] profile=~q parser=~q parser_load=~q parser_semantics=~q parser_wrapper=~q parser_lexicon=~q generation_output=~q parsing_output=~q parsing_tree_report=~q token_normalization=~q style=~q timeout_seconds=~q~n",
        [
            ProfileName,
            ParserName,
            ParserLoadFile,
...
write_parsing_output_header(S) :-
    ...
    format(S, "%~n", []),
    format(S, "%   gen_to_parse_parsing_case(CaseId, SemanticInput, GenerationStatus, GeneratedTokens, ParserInputTokens, ParsingStatus, RecoveredSemanticOutput).~n", []),
    format(S, "%~n", []),
    format(S, "% Meaning:~n", []),
