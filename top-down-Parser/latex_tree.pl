 ...
 * Doppellinien funktionieren nicht
 */
...
openstream(Xmax, Ymax, Tree, Stream) :-
	open('ltree.tex', write, Stream),
        format(Stream, "\\documentclass[12pt, amsfonts]{book}~n", []),
				format(Stream, "\\usepackage{amssymb, amsmath}~n", []),
				format(Stream, "\\usepackage{stmaryrd}~n", []),
				format(Stream, "\\usepackage{amsfonts}~n", []),
				format(Stream, "\\usepackage{graphics}~n", []),
				format(Stream, "\\usepackage{pict2e}~n", []),
				format(Stream, "\\newcommand{\\drawline}{\\Line}~n", []),
...
closestream(Stream) :-
        format(Stream, "\\end{picture}~n\\end{figure}~n\\end{document}~n", []),
	close(Stream).
